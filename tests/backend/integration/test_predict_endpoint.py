...
class TestPredictEndpoint:
    async def test_returns_lat_lon_for_valid_sample(self, api_client, db_session, created_sample, mock_ml_pipeline):
    ...
    async def test_returns_404_when_no_abundance(self, api_client, created_sample, mock_ml_pipeline):
        # Sample exists but has no abundance rows yet.
        response = await api_client.get(f"/samples/{created_sample.sample_name}/predict")
        assert response.status_code == 404
        assert "No abundance data" in response.json()["detail"] or "Has the pipeline completed" in response.json()["detail"]
    ...
    async def test_returns_500_when_model_crashes(self, api_client, db_session, created_sample, mock_ml_pipeline):
        await _seed_species_abundance(db_session, created_sample)
