...
def load_batch_data(batch_dir, level, mapper, base_dir):
    file_path = os.path.join(
        base_dir,
        f"{batch_dir}/short-read-taxonomy/final_reports/kraken_bracken_{level}.csv",
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Merge batch data for mixed samples.",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to raw_data_table_batch.csv",
    )
    parser.add_argument(
        "--levels",
        nargs="+",
        default=["phylum", "class", "order", "family", "genus", "species"],
        help="Taxonomic levels to merge.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="01_merged_output",
        help="Directory to save the merged csvs.",
    )
...
"""
python3 src/mixed_samples/01_get_data.py 
    --base_dir /lunarc/nobackup/projects/snic2019-34-3/shared_elhaik_lab1/Projects/Microbiome/Results/Mixed2025_results 
    --mapper_file data/mixed_samples/00_raw_data/raw_data_table_batch.csv 
    --output_dir data/mixed_samples/01_merged_data/"
"""
