...
def standardize_bracken(input_file: str, output_dir=None, min_abd: float = 0.001):
    ...
    abs_path = input_file.split(
        "/"
    )  # This works only if the file format follows the pipeline conventions
    sample = abs_path[-2]
    ...
    standardized_df.to_csv(
        os.path.join(output_dir, sample + "_" + rank + ".csv"), header=True, index=False
    )
